...
function Header() {
  ...
  const mobileNavigationRef = useInertRegion<HTMLDivElement>(!isMenuOpen);
    ...
    <>
      ...
      <header className="fixed top-2 left-0 right-0 z-50 h-24 flex items-center hidden md:block font-nav">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex justify-center">
          ...
          >
            {/* Unscrolled Layout */}
            <div
              ref={unscrolledHeaderRef}
              id="desktop-header-unscrolled"
              className={`
                flex w-full items-center justify-between
                transition-opacity duration-300
                ${scrolled ? 'invisible opacity-0 pointer-events-none' : 'visible opacity-100'}
              `}
            >
            ...
            <div
              ref={scrolledHeaderRef}
              id="desktop-header-scrolled"
              className={`
                absolute inset-0 flex items-center justify-between px-4 space-x-6
                transition-opacity duration-300
                ${scrolled ? 'visible opacity-100' : 'invisible opacity-0 pointer-events-none'}
              `}
            >
              <CompactHomeLink />
              {navLinks}
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out md:hidden ${scrolled || isMenuOpen ? 'bg-[var(--color-bg)]/80 backdrop-blur-md shadow-md' : 'bg-transparent'}`}>
        ...
        <div
          ...
          className={`absolute top-20 left-0 w-full bg-[var(--color-bg)] shadow-lg md:hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'visible transform translate-y-0 opacity-100' : 'invisible transform -translate-y-4 opacity-0 pointer-events-none'}`}
        >
